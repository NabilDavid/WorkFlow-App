...
{
    public class Talab_Azn_ShController : Controller
    {
        private WF_EN db = new WF_EN();


        // get data of AznGrid
        public JsonResult bind_data_Azn(string fromdate, string FirmCode)
        {
            ...
            string query = @"SELECT  PERSON_DATA.RANK_ID,
                              ...
                              WHERE (PERSON_DATA.PERSON_CODE = FIRMS_ABSENCES_PERSONS.PERSON_CODE)
                                    ...
                                    AND (firms_absences_persons.absence_type_id = 48))";
   ...
   ABSENCE_TYPE_ID, RANK_CAT_ID, PERSON_CAT_ID, 
    OFF_ABS_STEPS_ID, OFF_ABS_GROUP_ID ) 
      VALUES ('" + maxid + "' , '" + PERSON_CODE + "' , '" + fin_year + "' , '" + period_id + "' , '" + firms + @"' , 
      TO_TIMESTAMP ('" + FullDateFrom + @"', 'dd/mm/yyyy hh24:mi')
     ,TO_TIMESTAMP ('" + FullDateTo + @"', 'dd/mm/yyyy hh24:mi')
     ,TO_TIMESTAMP ('" + FullDateTo + @"', 'dd/mm/yyyy hh24:mi')
     , '" + ABSENCE_TYPE_ID + "' , '" + RANK_CAT_ID + "' , '" + person_rank_cat + @"' ,
  '" + data2.Tables[0].Rows[i].ItemArray[j] + "' , '" + data2.Tables[0].Rows[i].ItemArray[j + 1] + "'   )";
...
//                           AND PD.OUT_UN_FORCE = 0";
        ...
        // delete Azn order
        public int Dele_Azn_Order(string PERSON_CODE, string FIN_YEAR, int TRAINING_PERIOD_ID, string FullDateFrom, string FullDateTo, string FIRM_CODE, int ABSENCE_TYPE_ID, int RANK_CAT_ID, int PERSON_CAT_ID)
        {
        ...
        public int Dele_Azn_Order_Det(string PERSON_CODE, string FIN_YEAR, int TRAINING_PERIOD_ID, string FullDateFrom, string FullDateTo, string FIRM_CODE, int ABSENCE_TYPE_ID, int RANK_CAT_ID, int PERSON_CAT_ID)
        {
        ...
        public JsonResult firms_ddl(string firm_code)
        {
            string query = @"SELECT FIRM_CODE,NAME
                             FROM firms where FIRM_CODE='" + firm_code + "'";
        ...
        {
            string query = @"SELECT RANK_CAT_ID,NAME FROM RANK_CATEGORIES order by RANK_CAT_ID";
        ...
        // popub officers
        public ActionResult Create_Officers()
        {
            ...
            return PartialView("Create_Officers");
        }

        //
        // GET: /Talab_Azn_Sh/Details/5
