...
var ControllerName = ApplicationName + '/TRAVELS/';
var data_row;
var data_row1;
var group_id
var country_id;
...
$(document).ready(function () {
    ...
    $('#st_TAMAMM_gridDiv').on('rowselect', function (event) {
        ...
        mission_gehat = data.COUNTRY_NAME
        off_Id = data.PERSON_CODE;
        //$("#dialog-edit2").dialog('close');
        $('#ddl_name_txt').val(off_name);
        country_id = data.COUNTRY_ID;
        data_row1 = { PERSON_CODE: data.PERSON_CODE, COUNTRY_ID: data.COUNTRY_ID, TRAVEL_DATE: data.TRAVEL_DATE};
...
function st_TAMAMM_gridDiv_Refresh(firm_cod, date) {

    $('#st_TAMAMM_gridDiv').jqxGrid('clearselection');
    var source = {
        datatype: "json",
        datafields: [
      { name: 'TRAVEL_REASON_ID' },
      { name: 'TRAVEL_DATE' },
      { name: 'PERSON_CODE' },
      { name: 'TRAVEL_REASON' },
      { name: 'PERSON_NAME' },
      ...
      { name: 'END_DATE' },
      { name: 'RETURN_BAND_NO' },
      { name: 'RETURN_BND_DATE' },
      { name: 'RETURN_DATE' },
      { name: 'START_DATE' },
      { name: 'SUBJECT' },
      { name: 'TRAVEL_BAND_NO' },
      { name: 'TRAVEL_BND_DATE' },
      { name: 'COUNTRY_NAME' }

        ],
        async: false,

        url: 'TRAVELS/GET_travels_grids',
...
function build_static() {
    ...
    $('#page_name').text('السفريــات');
    $('#jqxTabs').jqxTabs({ width: '100%', height: 500, rtl: true, theme: 'darkblue' });
...
function build_TAMAMM_grid() {
    ...
    $("#st_TAMAMM_gridDiv").jqxGrid({
        ...
        columns: [
                                          { text: 'كود الرتبة', dataField: 'TRAVEL_REASON_ID', cellsalign: 'center', align: 'center', width: '10%', hidden: true },
                               { text: 'الرتبة', dataField: 'RANK', cellsalign: 'center', align: 'center', width: '5%' },//
                               { text: 'الاسم', dataField: 'PERSON_NAME', cellsalign: 'center', align: 'center', width: '20%' },//
                               { text: 'الدولة', dataField: 'COUNTRY_NAME', cellsalign: 'center', align: 'center', width: '15%' },//
                               { text: 'كود الوحدة', dataField: 'FIRM_CODE', cellsalign: 'center', align: 'center', width: 90, hidden: true },
                               { text: 'سبب السفر', dataField: 'TRAVEL_REASON', cellsalign: 'center', align: 'center', width: '10%' },//
                               { text: 'تاريخ السفر', dataField: 'TRAVEL_DATE', cellsalign: 'center', align: 'center', width: '10%' },//
                                 { text: 'السبب', dataField: 'NOTES', cellsalign: 'center', align: 'center', width: '25%' },//
...
function bnd_TAMAMM_grid(firm_cod) {
...
function bnd_Edit_time_grid(firm_code, fin_year, PERIOD_ID, date, person_id) {
    ...
    var source = {
        ...
        url: 'OFF_TAMMAM/GET_grid_OFF_TIM',
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            ...
                            if (gridId == "st_TAMAMM_gridDiv") {
                                ...
                                var xxx = $("#DT_DATE").val();
                                eval(gridId + "_Refresh(" + firm_cod + ",'" + xxx + "')");
...
function build_gehat_grid() {
    ...
    $("#gehat_grid").jqxGrid({
        ...
        columns: [
                       ...
                       { text: 'الجهة', dataField: 'COUNTRY_NAME', cellsalign: 'center', align: 'center', width: '100%' }
...
function bnd_gehat_grid() {
    ...
    var source = {
        ...
        url: 'TRAVELS/GET_grid_gehat_travels',
        data: {}
