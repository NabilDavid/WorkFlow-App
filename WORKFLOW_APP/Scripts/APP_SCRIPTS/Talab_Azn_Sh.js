//var ControllerName = ApplicationName + '/Talab_Azn_Sh/';
...
$(document).ready(function () {
    ...
    $('#TalabDate').on('change', function (event) {
        from_date = $('#TalabDate').val();
        bindAznGrid(from_date, firms);// keyboard, mouse or null depending on how the date was selected.
...
function bindAznGrid(fromdate1, FirmCode1) {
    ...
    var source = {
        ...
        url: 'Talab_Azn_Sh/bind_data_Azn',
        data: {
            fromdate: fromdate1,
            FirmCode: FirmCode1
        }
...
function show_Report_Sh() {
    ...
    var url = "REPORTS/Rep_Azn_Sh/Report_Azn_Sh.aspx?rank_off=" + rank_off + "&person_name=" + person_name + "&from_date=" + from_date + "&from_Time=" + from_Time + "&To_Time=" + To_Time + "&PERSON_CODE=" + PERSON_Co;
...
function bld_off_all_grd() {
    ...
    gridAddUrl = "Talab_Azn_Sh/" + 'Create_Officers';
    $("#off_grid").jqxGrid({
        width: 500,
        height: 300,
        // pageable: true,
...
function build_ROLE_grid() {
    ...
    theme = "darkblue";
    headerText = " الخطوات";
    gridAddUrl = "Talab_Azn_Sh/" + 'Create';
    $("#off_ROLE").jqxGrid({
        width: '80%',
        height: 200,
        //pageable: true,
...
function bnd_ROLE_grid(firms, fin_year, period_id, PERSON_CODE, RANK_CAT_ID, person_rank_cat, FullDateFrom, FullDateTo, ABSENCE_TYPE_ID) {
    ...
    var source = {
        ...
        url: 'Talab_Azn_Sh/retrieveSteps',
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data == 1) {
                    ...
                    var spl = splt[0];
                    from_date = spl;
                    bindAznGrid($('#TalabDate').val(), firms);
                } else if ((data == 2)) {
                    swal({
                        title: "خطأ ",
...
function DEL_Azn_order() {
    ...
    swal({
        ...
        $.ajax({
            url: "Talab_Azn_Sh/Dele_Azn_Order",
            data: JSON.stringify({
                PERSON_CODE: PERSON_CODE,
                FIN_YEAR: FIN_YEAR,
...
function DEL_Azn_order_Det() {
    ...
    $.ajax({
        url: "Talab_Azn_Sh/Dele_Azn_Order_Det",
        data: JSON.stringify({
            PERSON_CODE: PERSON_CODE,
            FIN_YEAR: FIN_YEAR,
            TRAINING_PERIOD_ID: TRAINING_PERIOD_ID,
