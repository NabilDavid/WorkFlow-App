...
var ControllerName = ApplicationName + '/KHEDEMA_CHANGE_OFF/';
var tp;
var firms;
...
$(document).ready(function () {
    ...
    BLD_DD("MON_DD", "../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_MON", '100%', 30, "darkblue", 'ID', 'NAME', 0);
    ...
    Get_fin_year();
    bld_khdma_grd();
    set_dd_p(pers_cod, firm);

    //bnd_khdma_grd($("#FIRM_DD").val(), $("#KHDMA_DD").val(), $("#MON_DD").val(), $("#TY_DD").val(), tp);
    $('#KHDMA_DD').on('select', function (event) {
        var item = args.item;
        var value = item.value;
        BLD1_DD('K_DT1', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: value, MON: $('#MON_DD').val(), PERS: person_c.split(',')[0], TY: $('#ty').val(), PY: $('#tp').val() });
        BLD1_DD('K_DT2', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: value, MON: $('#MON_DD').val(), PERS: $('#off2_data').val(), TY: $('#ty').val(), PY: $('#tp').val() });
    ...
    $('#MON_DD').on('select', function (event) {
        ...
        var value = item.value;
        BLD1_DD('K_DT1', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: $('#KHDMA_DD').val(), MON: value, PERS: person_c.split(',')[0], TY: $('#ty').val(), PY: $('#tp').val() });
        BLD1_DD('K_DT1', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: $('#KHDMA_DD').val(), MON: value, PERS: $('#off2_data').val(), TY: $('#ty').val(), PY: $('#tp').val() });
        bnd_khdma_grd($('#FIRM_DD').val(), $('#KHDMA_DD').val(), $('#MON_DD').text(), $('#ty').val(), $('#tp').val());
    });
...
function get_off(data, off) {

    if (off == 1) {
        $('#OFF1_RNK').val(data.RANK);
        $('#OFF1_NM').val(data.PERSON_NAME);
        $('#off1_data').val(data.PERSON_CODE);
        BLD1_DD('K_DT1', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: $('#KHDMA_DD').val(), MON: $('#MON_DD').val(), PERS: data.PERSON_CODE, TY: $('#ty').val(), PY: $('#tp').val() });
    ...
    BLD1_DD('K_DT2', '../WORKFLOW_APP/KHEDEMA_CHANGE_OFF/GET_O_K1', '100%', 30, 'darkblue', 'DT', 'DT', -1, { FIRM: $('#FIRM_DD').val(), TYP: $('#KHDMA_DD').val(), MON: $('#MON_DD').val(), PERS: data.PERSON_CODE, TY: $('#ty').val(), PY: $('#tp').val() });
...
function exc_exchng() {
    ...
    data = {
        FIRM: $('#FIRM_DD').val(), TYP: $('#KHDMA_DD').val(), PC1: $('#off1_data').val(), PC2: $('#off2_data').val(), F_DT: $('#K_DT1').val(), T_DT: $('#K_DT2').val(),
        Command: "add_exc"
    }
